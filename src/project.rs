...
/// Generate class definition source files (`src/classes/<class>/mod.rs`) from a [`ProjectSpec`].
...
mod tests {
    ...
    fn actor_file_contains_struct_and_impl() {
        let project = generate_project(&sample_spec());
        let actor = &project.files["src/classes/player_controller/mod.rs"];
    ...
    fn enemy_actor_wires_tick() {
        let project = generate_project(&sample_spec());
        let actor = &project.files["src/classes/enemy_ai/mod.rs"];
