...
/// Generate class definition source files (`src/classes/*.rs`) from a [`ProjectSpec`].
...
mod tests {
    ...
    fn generates_expected_files() {
        ...
        assert!(paths.contains(&"src/classes/player_controller.rs"));
        assert!(paths.contains(&"src/classes/enemy_ai.rs"));
    }

    #[test]
    fn actor_file_contains_struct_and_impl() {
        let project = generate_project(&sample_spec());
        let actor = &project.files["src/classes/player_controller.rs"];
    ...
    fn enemy_actor_wires_tick() {
        let project = generate_project(&sample_spec());
        let actor = &project.files["src/classes/enemy_ai.rs"];
    ...
    fn write_to_dir() {
        ...
        assert!(dir.join("src/classes/player_controller.rs").exists());
        assert!(dir.join("src/classes/enemy_ai.rs").exists());
    }
}
